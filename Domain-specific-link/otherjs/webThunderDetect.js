document.write('<link rel="stylesheet" type="text/css" href="http://pstatic.xunlei.com/js/css/xl_style.css" />');
/*
    ...
    function OnDownloadClick(sDownloadURL, sResName, sRefPage, sPid, isOpenNewWindow, sType, selectType) {
        ...
        if (rand(1) == 1 && sResName != "tcfc") { //show(sDownloadURL,sPid);
            ...
            if (typeof (isOpenNewWindow) != "boolean") {
                if (isNaN(isOpenNewWindow))
                    isOpenNewWindow = undefined;
                else
                    isOpenNewWindow = Number(isOpenNewWindow);
                if (isOpenNewWindow != 2 && isOpenNewWindow != 10)
                    isOpenNewWindow = undefined;
            }
 ...
 */
        ...
        addTask: function(cid, url, refer, name, stat) {
            var _addTask = [
                Delegate.create(this, function() {
                    this.__thunder.AddTask4(url, "", "", name, refer, -1, 0, -1, document.cookie, cid, stat);
                }), 
                Delegate.create(this, function() {
                    this.__thunder.AddTask3(url, "", "", name, refer, -1, 0, -1, document.cookie, cid);
                }), 
                Delegate.create(this, function() {
                    this.__thunder.AddTask2(url, "", "", name, refer, -1, 0, -1, document.cookie);
                }), 
                Delegate.create(this, function() {
                    this.__thunder.AddTask(url, "", "", name, refer, -1, 0, -1);
                })
            ];
    ...
    function productShow(sDownloadURL, sPid, sFlag, linkObj, iswindowOpen) {
        ...
        if (thunderObj) 
        {
            ...
            if (!fFXbPlugin) {
                ...
                var XHR = jQuery.ajax({
                    url: 'http://pingce.vip.xunlei.com/interface/check_resource4',
                    data: {
                ...
                function showHtml(rtnData) {
                    ...
                    var timer = setTimeout(function() {
                        ...
                        var dleft = (bodyObj.clientWidth / 2) - 492 / 2 + document.documentElement.scrollLeft + document.body.scrollLeft;
                        var dtop = bodyObj.clientHeight / 2 - 274 / 2 + document.documentElement.scrollTop + document.body.scrollTop;
                        jqPopDiv.css({left: dleft,top: dtop,position: 'absolute',zIndex: 99});
                        drop(jQuery('#fdiv h2'), 'fdiv');
                        jqPopDiv.find('table').css('font-size', '12px').css('width', '462px').css('color', '#333');
                        jqPopDiv.find('.hd,.bd').css('width', 490).css('overflow', 'hidden');
    ...
    function uniondown(style, movie) {
        try {
            var url = "http://pv.xmp.stat.xunlei.com/UPV?gs=uniondown&style=" + style + "&movie=" + encodeURIComponent(movie);
            var image = new Image();
            image.src = url;
        } catch (e) {
        }
